          ...
          {bannerData.map((banner, index) => (
            <CarouselItem key={index}>
              <div className="relative w-full aspect-[16/7] rounded-2xl overflow-hidden">
                <Image
                  src={`/placeholder.svg?width=1200&height=525&query=${encodeURIComponent(banner.imageQuery)}`}
                  alt={banner.title}
                  fill
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/50 to-transparent flex flex-col justify-center p-8 md:p-12 lg:p-16">
                  <div className="max-w-md lg:max-w-lg text-white">
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold drop-shadow-lg">{banner.title}</h2>
                    <p className="mt-2 md:mt-4 text-base md:text-lg drop-shadow-md">{banner.description}</p>
                    <Button
                      asChild
                      size="lg"
