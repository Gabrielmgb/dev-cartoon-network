...
import { Gamepad2, Play, BusIcon as SoccerBall, Globe, Smartphone, X } from "lucide-react"
...
const CNLogo = () => (
  <svg width="120" height="60" viewBox="0 0 120 60" fill="white" xmlns="http://www.w3.org/2000/svg">
    <rect width="55" height="55" fill="white" />
    <rect x="65" width="55" height="55" fill="black" />
    <path d="M20 10 H 35 V 20 H 25 V 45 H 35 V 55 H 20 V 10 Z" fill="black" />
    <path d="M75 10 H 90 V 20 H 80 V 25 H 95 V 40 H 80 V 45 H 90 V 55 H 75 V 10 Z" fill="white" />
    <text x="5" y="58" fontFamily="Arial, sans-serif" fontSize="8" fill="white" fontWeight="bold">
      CARTOON NETWORK
    </text>
  </svg>
)
...
export default function CartoonNetworkPage() {
  ...
  const characters = [
    ...
    { name: "Boy with purple hat", color: "bg-purple-300", query: "cartoon boy with purple hat" },
    { name: "Orange character", color: "bg-yellow-300", query: "orange cartoon character with antlers" },
    { name: "Hero girl", color: "bg-cyan-300", query: "female cartoon hero with green visor" },
    { name: "Robin", color: "bg-yellow-400", query: "cartoon robin superhero" },
    { name: "Grizzly Bear", color: "bg-pink-300", query: "brown grizzly bear cartoon" },
    { name: "Gumball", color: "bg-blue-300", query: "gumball cartoon character" },
    { name: "Soccer ball", color: "bg-yellow-500", query: "abstract soccer ball logo" },
    { name: "Bugs Bunny", color: "bg-red-400", query: "bugs bunny cartoon character" },
    { name: "Swirl Logo", color: "bg-gray-800", query: "black and white swirl logo" },
    { name: "Babs Bunny", color: "bg-yellow-200", query: "female bunny cartoon character" },
    { name: "Scooby Doo", color: "bg-pink-500", query: "scooby doo cartoon character" },
  ]

  const mainContent = [
    { query: "cartoon adventure game screenshot with fantasy characters in a forest" },
    { query: "colorful cartoon network banner with multiple characters and text" },
        ...
        <div className="container mx-auto px-4 flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navItems.map((item, index) => (
              <Link key={index} href="#" className="flex flex-col items-center space-y-1 group">
          ...
          <Carousel opts={{ align: "start", loop: true }} className="w-full">
            <CarouselContent>
              {characters.map((char, index) => (
                <CarouselItem key={index} className="basis-1/8 md:basis-1/12 lg:basis-1/14">
                  <div className="p-1">
                    <div
                  ...
                  <div className="p-1">
                    <Card className="overflow-hidden rounded-2xl">
                      <CardContent className="p-0">
                        <Image
                          src={`/placeholder.svg?width=400&height=225&query=${encodeURIComponent(content.query)}`}
