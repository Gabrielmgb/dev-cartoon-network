  ...
  const filteredAndSortedShows = useMemo(() => {
    const filtered = showsData.filter((show) => {
      const matchesCategory =
        filters.category === "Todos" || show.category === filters.category;
      const matchesCreator =
        filters.creator === "Todos" || show.details.creator === filters.creator;
      const matchesDecade =
        filters.decade === "Todos" || show.decade === filters.decade;
      const matchesSearch =
        filters.searchTerm === "" ||
        show.name.toLowerCase().includes(filters.searchTerm.toLowerCase()) ||
        show.details.creator
          .toLowerCase()
          .includes(filters.searchTerm.toLowerCase());

      return (
        matchesCategory && matchesCreator && matchesDecade && matchesSearch
      );
    });
    ...
    filtered.sort((a, b) => {
      switch (filters.sortBy) {
        ...
        case "year":
          return (
            Number.parseInt(b.details.year.split("–")[0]) -
            Number.parseInt(a.details.year.split("–")[0])
          );
        case "creator":
